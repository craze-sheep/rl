...
class Model(nn.Module):
    def __init__(self, action_shape):
        ...
        self.q_cnn = nn.Sequential(
            nn.Conv2d(5, 32, kernel_size=6, stride=2, padding=2),  # (5, 128, 128) -> (32, 64, 64)
            nn.ReLU(),

            nn.Conv2d(32, 64, kernel_size=3, stride=2, padding=1),  # (32, 64, 64) -> (64, 32, 32)
            nn.ReLU(),
            ResidualBlock((64, 32, 32), type='cnn'),

            nn.Conv2d(64, 128, kernel_size=3, stride=2, padding=1),  # (64, 32, 32) -> (128, 16, 16)
            nn.ReLU(),
            ResidualBlock((128, 16, 16), type='cnn'),
            ResidualBlock((128, 16, 16), type='cnn'),

            nn.Conv2d(128, 256, kernel_size=3, stride=2, padding=1),  # (128, 16, 16) -> (256, 8, 8)
            nn.ReLU(),
            ResidualBlock((256, 8, 8), type='cnn'),
            ResidualBlock((256, 8, 8), type='cnn'),
            nn.AvgPool2d(kernel_size=2),  # (256, 8, 8) -> (256, 4, 4)
        ...
        self.obstacles = nn.Parameter(torch.full((128, 128), -1.0), requires_grad=False)
    ...
    def forward(self, feature):
        ...
        # 更新全局障碍物地图信息 (-1表示不确定障碍物，0表示障碍物 1表示有障碍物)
        current_obstacles, _ = torch.max(map[:, 0], dim=0)  # (128, 128)
        with torch.no_grad():
            self.obstacles.data = torch.maximum(self.obstacles.data, current_obstacles)
        map[:, 0] = self.obstacles.data
...
class ResidualBlock(nn.Module):
    def __init__(self, dim: int | tuple, type='linear'):
        ...
        if type == 'linear':
            ...
            self.net1 = make_cnn_layer(dim[0], dim[0], init_method='kaiming')
            self.net2 = make_cnn_layer(dim[0], dim[0], init_method='kaiming')
