...
def workflow(envs, agents, logger=None, monitor=None):
    try:
        ...
        # usr_conf = read_usr_conf("agent_target_dqn/conf/train_env_conf_fixed.toml", logger)
        if usr_conf is None:
        ...
        while True:
            for g_data, monitor_data in run_episodes(episode_num_every_epoch, env, agent, usr_conf, logger, monitor):
                agent.learn(g_data)
                g_data.clear()
                # print(f"{'='*10}[DEBUG] Finish one episode{'='*10}")
                # exit()
...
def run_episodes(n_episode, env, agent: Agent, usr_conf, logger, monitor):
    try:
        for episode in range(n_episode):
            ...
            # Feature processing
            # 特征处理
            obs_data, _ = agent.observation_process(obs, extra_info=extra_info)
            ...
            while not done:
                ...
                # Feature processing
                # 特征处理
                _obs_data, reward = agent.observation_process(_obs, terminated, truncated, _extra_info)
                ...
                if done:
                    monitor_data = {
                        "diy_1": win_rate,
                        "diy_2": total_reward,
                        "diy_3": total_hit_wall,
                        "diy_4": int(np.sum(agent.state_manager.map_manager.explored == 0)),
                        # "diy_5": 0,  # 在algorithm上传epsilon系数
                    }
