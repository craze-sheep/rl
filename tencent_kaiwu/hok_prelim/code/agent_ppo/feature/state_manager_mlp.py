    ...
    def update_obstacles(self, hero_pos, map_info):
        ...
        for i in range(-25, 26):
            for j in range(-25, 26):
                u, v = i // 5 + 5, j // 5 + 5
                ii, jj = hero_pos[0] + i, hero_pos[0] + j
                if ii < 0 or ii >= 128 or jj < 0 or jj >= 128:
                    continue
                if self.obstacles[ii, jj] == 0:  # 如果已经是通路则无需更新了
                    continue
                self.obstacles[ii, jj] = map[u, v]
...
class StateManager:
    ...
    def get_reward(
        ...
        r -= float(np.sum(around_memory * cfg.REW_MEMORY_PUNISH_COEF))
    ...
    def get_obs(self, action_mask) -> np.ndarray:
        ...
        # 4. 可用动作 (16,)
        feature.append(action_mask)
        # 5. 终点位置 (6,)
        feature.append(self.end.get_feature(hero_pos))
        # 6. SEQUENCE_LENGTH前的英雄位置 (6,)
        _, prev_hero_pos = get_hero_info_and_pos(self.prev_frames[0])
        feature.append(get_position_feature(hero_pos, prev_hero_pos, found=True, available=True))
        # 7. buff位置 (6,)
        feature.append(self.buff.get_feature(hero_pos))
        # 8. treasure位置 (6,) * 13
        treasures_feature = []
        for i in range(13):
            treasures_feature.append(self.treasures[i].get_feature(hero_pos))
        treasures_feature = sorted(treasures_feature, key=lambda x: x[3])
        feature += treasures_feature
        # 10. 地图障碍物信息 (11*11,)
        x = [line['values'] for line in self.curr_frame['obs']['map_info']]
        feature.append(np.concatenate(x, dtype=np.float32))
        # 11. 记忆地图障碍物信息 (17*17,)
        feature.append(np.clip(
            self.map_manager.get_around_memory(17).reshape(-1) / 5,
    ...
    def get_action_mask(self) -> np.ndarray:
        ...
        return mask, delta_distance, hit_wall
